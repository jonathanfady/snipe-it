<div id="{{ $fieldname }}" class="form-group {{ $errors->has($fieldname) ? 'has-error' : '' }}">
    ...
    <div class="col-md-7 @if(isset($item) && \App\Helpers\Helper::checkIfRequired($item, $fieldname)) required @endif">
        <select class="js-data-ajax" data-endpoint="suppliers" data-placeholder="{{ trans('general.select_supplier') }}"
            name="{{ $fieldname }}" style="width: 100%" id="supplier_select" aria-label="{{ $fieldname }}"
    ...
    <div class="col-md-1 col-sm-1 text-left">
        ...
        <a href='{{ route('modal.show', 'supplier') }}' data-toggle="modal" data-target="#createModal"
            data-select='supplier_select' class="btn btn-sm btn-primary">New</a>
        @endif
        @endcan
    </div>
