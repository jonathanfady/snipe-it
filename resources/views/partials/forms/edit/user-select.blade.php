<div id="{{ $fieldname }}" class="form-group {{ $errors->has($fieldname) ? 'has-error' : '' }}">
    ...
    <div class="col-md-6 @if((isset($required)) && ($required=='true')) required @endif">
        <select class="js-data-ajax" data-endpoint="users" data-placeholder="{{ trans('general.select_user') }}"
            name="{{$fieldname}}" style="width: 100%" id="{{$fieldname}}_id_select" aria-label="{{$fieldname}}" {!!
            isset($activated_users_only) ? 'data-user-activated="1"' : '' !!}>
